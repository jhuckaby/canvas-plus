	...
	text: function(opts) {
		...
		if (overflow.match(/wrap/i)) {
			...
			if (overflow.match(/shrink/i)) {
				...
				while (((lines.length * line_height) - opts.lineSpacing > temp_avail_height) || lines._wordChopped) {
		...
		var renderText = function(ctx, width, height, mx, my, dx, dy) {
			...
			for (var idx = 0, len = lines.length; idx < len; idx++) {
				...
				if (kerning != 0) {
					...
					// note that node-canvas only measures widths as integers, so this is problematic
					...
					// for default kerning, render entire line at once
					if (opts.outlineColor) ctx.strokeText(line, x, y);
		...
		if (opts.autoCrop) {
			...
			if (gravity.match(/(north|top)/i)) cy = 0;
					...
					if (word.length) {
						...
						if (line_width + word_width > avail_width) {
							...
							if (idy == 0) {
								...
								// add hint that we chopped
								lines._wordChopped = true;
							} // first word on line
							else {
